...
- **Build an agent on the wrong surface and it inherits a named human's permissions.** Same vendor, same directory underneath, three different answers depending on which door you walked through. Your audit log cannot tell the agent from the person. [Where the asymmetry bites]({{ '/docs/scenarios#scenario-10-agent-governance-and-agent-sprawl' | relative_url }})
