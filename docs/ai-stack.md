...
2. **It's one of seven.** Microsoft currently runs several concurrent agent taxonomies: this archetype framework, CAF's agent types, declarative-vs-custom-engine, the Agent Registry publisher types, the Integrated Apps grouping, and Agent 365's "types of agents." **None supersedes another and none cross-references the others.** Pick the one that fits the conversation you're in, and say which one you picked.
