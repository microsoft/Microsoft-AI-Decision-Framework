...
{: .warning }
> **Pick the harness deliberately, because the pick does not reverse.** Copilot Studio agents are created on one of three harnesses, and Microsoft states that an agent created on the GitHub Copilot harness *"can't be transferred to the standard harness, and vice versa."* The trade is the familiar one: the standard harness gives you explicit, rule-based conversation control, while the GitHub Copilot harness reasons across files, tools, and knowledge but decides more of its own path. Neither is deprecated. **Decide this before you build, not after**, and see [Technologies]({{ '/docs/technologies' | relative_url }}) for current status, capability differences, and the billing distinction between them.
...
**But read the constraint carefully, because it is the part that matters for planning.** This is a choice you make when you create the agent, not a dial you turn afterward. Microsoft states that agents created on the GitHub Copilot harness *"can't be transferred to the standard harness, and vice versa."* So the harness is selectable and the selection is durable. **That combination is unusual and worth internalizing:** you get a real architectural decision, exactly once, at the moment you click create, before you know most of what you will learn about the problem.

Watch this pattern rather than the product detail. When a layer that used to be invisible becomes a named choice, three questions follow: what does picking it cost you later, what can it not do yet, and does a door back exist. Those questions survive the next release.
