...
{: .warning }
> **Copilot Studio now has two experiences, and for any given agent the choice does not reverse.** The classic experience gives you explicit conversation flows with conditions, branching, and node-based logic. The new experience replaces that with instructions and reasoning, which trades step-by-step determinism for adaptability. Microsoft's own guidance is that if you need *"precise control over every step of a conversation, the classic experience provides more deterministic options."* You can move between the experiences freely as a maker, but an agent built in one cannot be carried into the other. **This is a fork you choose deliberately, not a migration you schedule.** See [Technologies]({{ '/docs/technologies' | relative_url }}) for current status, conversion mechanics, and the capability gaps on each side.
...
**A note on vocabulary, because this one is genuinely confusing and the confusion is instructive.** Nobody agrees on the word. Researchers say *agent-computer interface*. Practitioners say *harness*. GitHub calls its SDK a *"production-tested agent runtime."* Microsoft Agent Framework publishes the clearest definition of the lot: *"An agent harness is the scaffolding that turns a language model into an agent that can actually do things... you need a runtime wrapped around the model, and that runtime is the harness."*

Then there is Copilot Studio, where Microsoft's own teams disagree in public. The product documentation says **new agent experience** and **enhanced orchestration runtime** and never uses the word *harness* at all. The engineering team's blog announces the same thing as the **GitHub Copilot harness**, and publishes a post headlined *"New Harness, New Rules?"* at a URL that reads *new-orchestrator-resources*. Same technology, three vocabularies, one company.

**This is not a criticism, it is a working condition.** Find this layer in any architecture diagram regardless of what it is called, because it determines more of the system's behavior than the box marked "model." And when you search documentation, search for the concept rather than the word.
...
That bundle is loosening, but not evenly, and the difference matters more than the trend. **Model choice has genuinely become a setting**: Copilot Studio lets you pick the model an agent reasons with. **The harness has not.** What is happening instead is more interesting and more demanding: vendors are *replacing* the loop underneath products that never exposed one. Copilot Studio's newer agent experience runs on a rebuilt orchestration runtime, and Microsoft is explicit that it is not a dial, saying the new experience uses that runtime *"for all agents, with no option to switch."*

**So the honest version is this: you do not choose a harness, you choose an experience, and the harness arrives with it.** That is a heavier decision than a dropdown, because the experiences are not interchangeable. Microsoft states that agents built in one *"can't be converted"* to the other, in either direction. A runtime you cannot select and cannot leave is not a setting. It is an architectural commitment made at the moment you click *create*.

Watch this pattern rather than the product detail. When a platform's engine is swapped rather than exposed, the interesting questions are what the old artifacts do (usually: keep running, unconverted), what the new engine cannot do yet, and whether the door back exists. Those are the questions that survive the next release.
