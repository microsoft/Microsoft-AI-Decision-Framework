...
Notice what does *not* change as you climb: the bucket. Climbing is not how you move from *AI for You* to *AI for Your Codebase*. Those are separate conversations, and climbing is how far you descend into the stack **within** the conversation you're already in.

**But artifacts can graduate between buckets, and that motion is worth naming because it is how adoption usually starts.** One person solves their own problem and builds something small and reusable to do it. It earns its keep by getting used. When the team needs the same thing, that artifact is promoted into a governed, owned, supported capability serving people who never built anything. **This is promotion, not climbing.** The conversation genuinely changes when it happens: a personal shortcut answers to nobody, while a team capability acquires an owner, a review, and someone accountable at 2 a.m. Plan for the promotion rather than being surprised by it, and build the reusable part so it can travel. A developer bucket has its own SaaS rung (Copilot in the editor) and its own IaaS rung (your own harness on your own compute), and an employee-productivity bucket has both too. Same ladder, different building.
