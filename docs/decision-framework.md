...
**Meter 1: Copilot Credits (the Microsoft 365 side).** Pay-as-you-go at **$0.01 per credit**. This is what metered agent usage draws down for Copilot Chat and agent scenarios in Microsoft 365. **One detail that catches pilots:** on Copilot Studio's GitHub Copilot harness, credits are consumed *"from the moment you start building"*, covering the model, the tools, and the harness itself, whereas the standard harness starts billing after publish. Budget the experiment, not just the deployment.
