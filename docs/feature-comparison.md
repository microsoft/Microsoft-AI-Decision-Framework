...
[^mobile-ext]: Microsoft 365 Copilot release notes (Nov 25, 2025): mobile support for custom engine agents and message-extension agents on iOS/Android. Source: [Microsoft 365 Copilot release notes - Nov 25, 2025](https://learn.microsoft.com/en-us/microsoft-365/copilot/previous-year-release-notes#november-25-2025) (Retrieved: 2025-01-27).
[^agent-registry]: Agent Registry lifecycle actions (publish, deploy, pin, block, delete, export inventory). Current documentation doesn't describe activating an agent or reassigning its owner; where an agent has no owner, the documented remedies are blocking or deleting it. Source: [Agent Registry in the Microsoft 365 admin center](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/agent-registry?view=o365-worldwide) (Retrieved: 2026-08-31).
...
[^p3-licensing]: **Two prepurchase plans, not one.** The Copilot Credit Pre-Purchase Plan buys CCCUs against eligible Copilot Credit usage; the Microsoft Agent Prepurchase Plan buys ACUs for select services across Microsoft Foundry, Copilot Studio, Fabric, and GitHub. Both have one-year terms, auto-renew on by default, and all purchases final. One ACU pays down US$1 of qualifying retail cost, purchased at a tiered discount. Reservations apply before prepurchase plans; the narrower Copilot Credit plan applies before the Agent plan. [Learn's spending-policy page](https://learn.microsoft.com/en-us/microsoft-365/copilot/usage-based-billing-manage-copilot-credits) now lists *"Copilot Credit Pre-purchase plans (P3)"* for policies *"currently available for Cowork and Work IQ API"*. It spends prepaid credits before pay-as-you-go. Neither that page nor the Agent plan names Code or Copilot Managed Runtime for prepurchase coverage as of 2026-09-25. Sources: [Copilot Credit Pre-Purchase Plan](https://learn.microsoft.com/en-us/azure/cost-management-billing/reservations/copilot-credit-p3), [Microsoft Agent Prepurchase Plan](https://learn.microsoft.com/en-us/azure/cost-management-billing/reservations/agent-pre-purchase).
...
[^gh-app-modes]: These are session modes inside the GitHub Copilot desktop app. Distinguish its Autopilot mode from the Copilot app's Autopilot (formerly Microsoft Scout), Foundry's identity-based autopilot category, and Windows Autopilot device provisioning. See [Technologies: Autopilot]({{ '/docs/technologies#autopilot' | relative_url }}) for those naming boundaries. Source: [GitHub Copilot app](https://docs.github.com/en/copilot/concepts/agents/github-copilot-app).
[^copilot-sdk-seam]: The [GitHub Copilot SDK GA announcement](https://github.blog/changelog/2026-06-02-copilot-sdk-is-now-generally-available/) confirms Python, TypeScript, Go, .NET, Java, and Rust bindings; the [quickstart](https://docs.github.com/en/copilot/get-started/sdk-quickstart) demonstrates setup. [BYOK documentation](https://docs.github.com/en/copilot/how-tos/copilot-sdk/auth/byok) lists OpenAI, Microsoft Foundry, and Anthropic providers and `bearerTokenProvider` callbacks. The [Azure managed identity guide](https://docs.github.com/en/copilot/how-tos/copilot-sdk/setup/azure-managed-identity) documents Entra/managed identity through composition with the Azure Identity SDK. The [SDK README](https://github.com/github/copilot-sdk) says BYOK is key-only and does not support Entra or managed identities, which conflicts with that dedicated guide. Follow the dedicated guide and verify your selected SDK version. The [Foundry Agent Service overview](https://learn.microsoft.com/en-us/azure/foundry/agents/overview) lists the Copilot SDK among hosted-agent frameworks, while [Foundry hosted agents](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agents) states: *"Hosted agents support Python and C#."* (Verified: 2026-09-14).
...
## App Building Comparison

**Choose for the next owner, too.** Compare who builds the app, where it runs, and how a developer takes over. A quick first build and a workable handoff are different requirements.

| Build path | Who builds | Where it runs | Governed from | Status | Billing | When it outgrows its builder |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Copilot Code** | Copilot users describing an app, tracker, dashboard, automation, or workflow | Desktop widgets and dashboards, or cloud-hosted internal apps on the Managed Runtime foundation (Official Microsoft Blog post) | No Learn feature overview found as of 2026-09-25; check hosted-app controls separately | **Frontier rollout announced:** the same post gives *"at the end of the month"* and *"in the coming weeks"*; the runtime post calls entry points *"already live"*. See [rollout details]({{ '/docs/technologies#the-new-copilot-app-september-2026-read-the-status-column-first' | relative_url }}) | USL plus UBB (Copilot Credits), documented on Learn | Not described on the Learn pages checked as of 2026-09-25 |
| **Cowork App skill** | Business users in a Cowork session, no code | Copilot Managed Runtime | Microsoft 365 admin center (**Apps**); Frontier user access decides who can build | **Preview**, Frontier only | Cowork runs on UBB; running the app is a separate bill[^cmr-run] | **Same repository**, in the blog's own example: a developer *"can pull the code down"* and keep building *"without forking"*[^same-repo] |
| **Copilot Studio apps** | Makers refining build, preview, and publish steps; the code view is read-only | Copilot Managed Runtime | Microsoft 365 admin center (**Apps**); app creation is on by default and admins can turn it off | **Preview**, rolling out gradually | The GitHub Copilot harness bills from creation, including testing; runtime is separate[^cmr-run] | A Studio-specific handoff is not described as of 2026-09-25. Learn says the paths *"produce the same kind of artifact"*[^same-repo] |
| **Managed Runtime SDK and CLI** | Professional developers with `ms` and `@microsoft/managed-apps`, directly or through the coding-agent plugin; the blog also names Lovable as a third-party builder | Copilot Managed Runtime | Microsoft 365 admin center (primary), plus Power Platform admin center; CLI app creation is off by default | **Preview** | Runtime entitlement applies per user, including developers running locally[^cmr-run] | Already in the repository, which Learn calls *"the single source of truth"* |
| **Power Apps vibe** | Business analysts, citizen developers, and professional developers | Power Platform, not in a default environment; US, Australia, Asia, and India regions only, English only | Power Platform admin center; *Copilot in Power Apps (preview)* must be on | **Preview.** The overview recommends Cowork and Copilot Studio app building *"moving forward"* | Not stated on the overview as of 2026-09-25 | **Fork:** conversion creates a new code app, disconnected from the original[^vibe-fork] |
| **Power Apps code apps** | Professional developers in a code-first IDE (React, Vue, and others) | Power Platform | Power Platform admin center; admins turn code apps on per environment | **Status not stated** on the overview as of 2026-09-25; no preview banner is not a GA label | End users need Power Apps Premium, pay-as-you-go, an App Pass, or auto-claim | Already code |
| **Power Apps canvas and model-driven, AI-assisted** | Makers and developers using a coding agent: the canvas authoring plugin or the model-driven app-builder skill | Power Platform | Power Platform admin center | **GA per the September 2026 blog:** the canvas authoring agent plugin and model-driven app-builder skill | Not stated in the announcement as of 2026-09-25 | Not stated in the announcement as of 2026-09-25 |

{: .note }
> As of 2026-09-25, the checked Microsoft pages do not provide a chooser between Copilot Managed Runtime and Power Apps code apps. Validate the data model and operating controls you need; do not assume a migration path. Learn does give persona guidance within Managed Runtime, and Power Apps vibe's overview recommends Cowork and Studio app building *"moving forward"*.

[^cmr-run]: **Running is its own bill.** Runtime users need Copilot Credits or Power Apps Premium, not a Microsoft 365 Copilot license. Credits are charged per launch and per API call; each API call consumes 0.1 credits. Power Apps Premium avoids those runtime credits except for separately billed services such as Work IQ APIs or usage above its API request limits. Build charges remain separate: Cowork needs a USL plus metered usage; Studio app creation bills through the GitHub Copilot harness before publication. Sources: [SDK overview (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/), [admin overview (preview)](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/), [FAQ (preview)](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/faq-about-apps) (Verified: 2026-09-25).
[^same-repo]: **"Same repository" rests on a blog example.** In Microsoft's Managed Runtime announcement, a team builds a launch-management app in Copilot Cowork, and *"A developer can pull the code down to continue to build upon what the team created with familiar web technologies and update it without forking or standing up a separate runtime, platform resources, and governance stack."* Learn documents the parts rather than the handoff: *"Every app has a repository associated with it"*, and all the creation paths *"produce the same kind of artifact"*. Sources: [Build where you want, run with confidence](https://www.microsoft.com/en-us/copilot/blog/copilot-studio/build-where-you-want-run-with-confidence-now-microsoft-hosts-and-manages-the-code-created-by-copilot/) (Published: 2026-09-25); [SDK overview (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/); [What is Copilot Managed Runtime (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/) (Verified: 2026-09-25).
[^vibe-fork]: **The fork, in Learn's words:** *"Converting a vibe app creates a new code app. The code app is disconnected from the original plan. Changes that you make to the code app don't appear in the vibe app or its plan."* The converted app is a Power Apps code app, published with the Power Apps CLI. Source: [Convert a vibe app to a code app](https://learn.microsoft.com/en-us/power-apps/vibe/convert-vibe-to-code) (Updated: 2026-09-04).

**Sources:** [What is Copilot Managed Runtime (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/); [Copilot Managed Runtime architecture (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/architecture); [Use Copilot Cowork](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/use-cowork); [Apps overview (preview)](https://learn.microsoft.com/en-us/microsoft-copilot-studio/apps-experience/apps-overview); [Copilot Studio apps FAQ](https://learn.microsoft.com/en-us/microsoft-copilot-studio/faq-apps); [USL and UBB](https://learn.microsoft.com/en-us/microsoft-365/copilot/user-subscription-license-usage-based-billing); [Power Apps code apps overview](https://learn.microsoft.com/en-us/power-apps/developer/code-apps/overview); [Overview of the Power Apps vibe experience (preview)](https://learn.microsoft.com/en-us/power-apps/vibe/overview); [Convert a vibe app to a code app](https://learn.microsoft.com/en-us/power-apps/vibe/convert-vibe-to-code); [Introducing the new Copilot with Home, Code and Autopilot](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/); [What's new in Power Platform: September 2026](https://www.microsoft.com/en-us/power-platform/blog/power-apps/whats-new-in-power-platform-september-2026-feature-update/) (Verified: 2026-09-25). Full limits: [Technologies: Copilot Managed Runtime]({{ '/docs/technologies#copilot-managed-runtime' | relative_url }}).
...
The [Microsoft IQ hub](https://learn.microsoft.com/en-us/microsoft-iq/) lists *"four interconnected capabilities"*. [What is Foundry IQ?](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/what-is-foundry-iq), updated 2026-08-01, lists *"three IQ workloads"*, omitting Web IQ. This table follows the hub's four-part list.
...
| **Status** | Partial GA (agentic retrieval **GA via the `2026-04-01` REST API**; portal experiences, answer synthesis, and multi-turn remain Preview) | **APIs GA (2026-06-16)**: Chat, Context, Tools, Workspaces. ⚠️ **Work IQ MCP is Preview**; do not conflate the two | Limited Access (enrollment required) | **Preview** workload on Learn (*"the IQ (preview) workload"*; ontology preview). **GA on Learn** for [Power BI data answering in Copilot Chat](https://learn.microsoft.com/en-us/fabric/iq/connectors/microsoft-365-copilot-overview), the [Fabric IQ plugin in Cowork](https://learn.microsoft.com/en-us/fabric/iq/connectors/cowork-overview), and [Fabric IQ MCP](https://learn.microsoft.com/en-us/fabric/iq/connectors/fabric-iq-mcp) |
...
- [Fabric IQ workload overview (Preview)](https://learn.microsoft.com/en-us/fabric/iq/overview)
...
- [Fabric IQ workload (Preview)](https://learn.microsoft.com/en-us/fabric/iq/overview)
...
**Confidence Level:** High for GA capabilities (Fabric data agent, pgvector, Azure Managed Redis); Medium for Preview capabilities (`VECTOR_SEARCH`, Fabric IQ workload, AI Search Serverless) and for Cosmos DB vector search, where Microsoft states no GA date
