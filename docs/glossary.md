...
**Agent2Agent (A2A)**
A protocol enabling secure, peer-to-peer communication between AI agents, allowing them to discover peers, negotiate tasks, and collaborate without centralized intermediaries ([A2A Protocol](https://a2a-protocol.org/latest/), verified 2026-08-10). A2A is an open industry standard rather than a Microsoft one; Microsoft implements it, and the specification is the authority on what it does.
...
**NLWeb**
An open project where web endpoints act as MCP servers, allowing websites to provide conversational interfaces and be easily discoverable by AI agents ([NLWeb](https://github.com/nlweb-ai/NLWeb), verified 2026-08-10). The project now lives in its own `nlweb-ai` GitHub organization rather than under `microsoft`; it is open source under MIT, and Microsoft is a participant rather than the sole owner.
...
A managed platform for geospatial insights that lets customers ingest, manage, search, and distribute private geospatial data within Azure through a **GeoCatalog** resource built on the open STAC specification, and is compatible with enterprise and open-source geospatial tooling ([Microsoft Planetary Computer Pro](https://learn.microsoft.com/en-us/azure/planetary-computer/microsoft-planetary-computer-pro-overview), verified 2026-08-10). ⚠️ **What we will not tell you:** that it integrates with Fabric. Earlier versions of this entry said so; the product documentation does not.
