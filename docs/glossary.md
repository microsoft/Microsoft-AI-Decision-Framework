...
Microsoft's first Autopilot agent - an always-on personal agent that works autonomously in the background to coordinate work across Teams, Outlook, OneDrive, and SharePoint without needing to be prompted each time. Scout proactively schedules meetings, generates prep materials, blocks calendar time for deliverables, and surfaces risks like stalled decisions. It operates under a governed Entra identity, enforces Purview policies, and is powered by Work IQ and OpenClaw open-source technology. Available as an experimental release through the Frontier program; requires Frontier enrollment, Intune policy configuration, opt-in attestation, and a GitHub Copilot license. Not a general productivity copilot (use M365 Copilot for prompt-driven work) and not a custom build target (use Copilot Studio or Foundry for custom agents). See [Introducing Microsoft Scout](https://www.microsoft.com/en-us/microsoft-365/blog/2026/06/02/introducing-microsoft-scout-your-always-on-personal-agent/) (published 2026-06-02) and [setup instructions](https://learn.microsoft.com/microsoft-scout).
