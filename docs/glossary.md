...
**Agent**
An AI system that uses an LLM to interpret user inputs, plan, call tools or MCP servers, and return responses, with optional threads, memory, and middleware to enrich interactions ([Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/overview/), updated 2026-02-20).
...
**Agent Framework**
An open-source development kit for .NET and Python (v1.0 GA, April 2026) that unifies Semantic Kernel and AutoGen concepts, adding stateful workflows, multi-agent orchestration, multi-provider service connectors, and MCP support for production-grade AI solutions ([Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/overview/); [v1.0 announcement](https://devblogs.microsoft.com/agent-framework/microsoft-agent-framework-version-1-0/), published 2026-04-03).
...
**Autopilot**
{: #autopilot }
An identity-based agent category in Foundry, and a personal-agent product name in Copilot. Learn defines a Foundry autopilot as *"a type of agent that works under its own identity as a persistent, named member of your organization"*. Its binary test is *"An agent either has its own agent user account or it doesn't"*. Autonomy alone does not qualify. The [Foundry definition](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/autopilot-overview) carries no status banner as of 2026-09-25; the [quickstart](https://learn.microsoft.com/en-us/azure/foundry/agents/how-to/agent-365) requires Frontier preview enrollment and an Agent 365 Frontier license for each instance.

The Copilot app's **Autopilot, formerly Microsoft Scout**, is described as cloud-hosted with *"its own identity"* in Jared Spataro's 2026-09-25 [Official Microsoft Blog post](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/), which says it is *"expanding to private preview at the end of the month"*. The two uses align on identity, but Microsoft has not stated whether this product is an instance of the Foundry definition as of 2026-09-25. Scout's documented desktop prerequisites do not establish Autopilot's requirements. See [Technologies: Autopilot]({{ '/docs/technologies#autopilot' | relative_url }}).
...
Choosing which model runs a prompt, rather than merely supplying prompt instructions. For prompts in Copilot Studio (standard harness), Power Apps, and Power Automate, this means connecting a supported model deployed in Microsoft Foundry and selecting it for each prompt. The connection requires a chat-completions endpoint; Responses API endpoints are not supported. The GPT-5 family and later models are not currently supported for BYOM in prompts ([Bring your own model for your prompts](https://learn.microsoft.com/en-us/microsoft-copilot-studio/bring-your-own-model-prompts), verified 2026-09-14). *When to use:* Consider BYOM when you need a supported model outside the managed defaults; if only the tone or format needs work, start with the prompt. See [Feature Comparison]({{ '/docs/feature-comparison' | relative_url }}) for platform-specific capabilities.
...
**Citizen developer**
{: #citizen-developer }
Business people who build applications outside a professional development role. Gartner defined the term in a 2009 release featuring analyst Eric Knipp: *"a user operating outside of the scope of enterprise IT and its governance who creates new business applications for consumption by others either from scratch or by composition"* ([October 22, 2009 release, archived](https://web.archive.org/web/20130128074257/http://www.gartner.com/it/page.jsp?id=1212813)).

The label remains contested. Matt Healy argues for retiring it in [The New Stack](https://thenewstack.io/its-time-to-kill-the-term-citizen-developer/) (2024-12-05), while [Power Apps vibe (Preview)](https://learn.microsoft.com/en-us/power-apps/vibe/overview) names *"Business analysts, citizen developers, and professional developers"* as its audience. A job label does not tell you whether an app has effective governance or a support owner. See [End-user programming](#end-user-programming).
...
{: #copilot-agent-runtime }
GitHub's name for the production harness backing its Copilot CLI, app, and SDK. GitHub also names VS Code, Visual Studio, Copilot Cowork, Copilot Studio, and Office apps among the products it backs. The article says most listed products replaced their earlier agent loops with the GitHub Copilot SDK, but does not identify which ones as of 2026-09-25. Use each product's documented controls; a shared harness does not establish identical versions, behavior, or exposure ([Migrating the GitHub Copilot runtime to Rust, using Copilot](https://github.blog/ai-and-ml/generative-ai/migrating-the-github-copilot-runtime-to-rust-using-copilot/), published 2026-09-16, updated 2026-09-23). This is not the [Copilot Managed Runtime](#copilot-managed-runtime) hosting service. See [Harness](#h).
...
**Copilot Code**
{: #copilot-code }
The natural-language app-building capability in the Copilot app. The 2026-09-25 [Official Microsoft Blog post](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/) by Jared Spataro describes work ranging from desktop widgets to cloud-hosted internal apps, using *"the same underlying technology as GitHub Copilot"*. The post gives both end-of-month and coming-weeks Frontier rollout timing; the runtime announcement calls the entry points *"already live"*. Learn documents Code's UBB but no Learn feature overview was found as of 2026-09-25. See [Technologies for the rollout statements]({{ '/docs/technologies#the-new-copilot-app-september-2026-read-the-status-column-first' | relative_url }}). Distinct from GitHub Copilot and its code-review feature.

**Copilot Cowork (GA)**
Long-running, multi-step delegated work inside Microsoft 365 Copilot. Hand over a job and steer its plan at checkpoints. [Generally available worldwide since 2026-06-16](https://www.microsoft.com/en-us/copilot/blog/2026/06/16/copilot-cowork-is-now-generally-available/). It requires a user subscription license plus Copilot Credits. The [FAQ](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/cowork-faq) meters activity, including model responses, tool and skill calls, image generation, and browser tasks. It limits use and access to Anthropic-supported regions, while the [access page](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/cowork-access) says disabling Anthropic models *"doesn't change whether the user can use Cowork"*. [Purview](https://learn.microsoft.com/en-us/purview/ai-copilot-cowork) marks data loss prevention as not supported (✕) for Cowork AI interactions.

The [model list](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/cowork-models), updated 2026-09-14, includes Auto (default), GPT 5.5 (Frontier), GPT 5.6 Sol, GPT 5.6 Terra, GPT 6 Astra, Opus 5, Claude Sonnet 5, and Claude Fable 5.1. Higher reasoning effort uses more credits.

The **App skill (Frontier, preview)** builds apps on [Copilot Managed Runtime](#copilot-managed-runtime). It is called *App skill* on the [Cowork page](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/use-cowork), *App builder* skill in the runtime overview, and */app skill* in the [2026-09-10 app-building blog](https://www.microsoft.com/en-us/copilot/blog/copilot-studio/build-apps-in-copilot-cowork-and-copilot-studio/). Do not confuse it with Power Platform's app-builder skill for model-driven apps. Before sharing, check the conflicting data-access and recipient-license statements in [Technologies]({{ '/docs/technologies#copilot-managed-runtime' | relative_url }}).
...
A Microsoft-operated, IT-governed host for internal apps, whether the builder describes the app or writes its code. Learn documents Cowork, Copilot Studio, and the SDK/CLI as creation paths; the announcements also name Copilot Code. The artifact is a TypeScript or JavaScript single-page app backed by Git, using Entra sign-in and environment-group policies. Learn labels the runtime *"a preview feature"*. It is a governed host, not a completed governance review: effective connector policy is conditional, the inventory is not a complete access audit, and Purview does not audit code changes. See [Technologies]({{ '/docs/technologies#copilot-managed-runtime' | relative_url }}) for limits and separate build/run billing ([runtime overview](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/), updated 2026-09-25).
...
{: #end-user-programming }
People who are not professional programmers building software for their own work. Spreadsheets are a familiar example. Bonnie Nardi studied spreadsheets and CAD systems in *A Small Matter of Programming: Perspectives on End User Computing* ([MIT Press](https://doi.org/10.7551/mitpress/1020.001.0001), 1993); Hermans and colleagues examined spreadsheet engineering in *Spreadsheets are Code* ([IEEE SANER paper](https://ieeexplore.ieee.org/document/7476773), 2016).

See [The Spreadsheet Precedent]({{ '/docs/capability-model#everyone-has-a-codebase-now' | relative_url }}) for the ownership problem: a small tool can become a team dependency before anyone agrees who maintains it. Related terms: [citizen developer](#citizen-developer) and [vibe coding](#vibe-coding).
...
**Fabric IQ (Preview workload; GA integrations)**
The semantics layer for the data estate: the vocabulary that turns a table named `CUST_MSTR` into "a customer." Learn labels the [workload](https://learn.microsoft.com/en-us/fabric/iq/overview) *"the IQ (preview) workload"* and its ontology item preview. [Power BI data answering in Copilot Chat](https://learn.microsoft.com/en-us/fabric/iq/connectors/microsoft-365-copilot-overview) and the [Fabric IQ plugin in Cowork](https://learn.microsoft.com/en-us/fabric/iq/connectors/cowork-overview) are each *"a generally available (GA) feature of Microsoft Fabric"* (both updated 2026-09-23); [Fabric IQ MCP](https://learn.microsoft.com/en-us/fabric/iq/connectors/fabric-iq-mcp) is also generally available. Jared Spataro's 2026-09-25 Official Microsoft Blog post agrees on Chat and Cowork and adds Code integration *"coming through the Frontier program soon"*.
...
{: #golden-path }
A supported default for building and shipping software. Netflix's *paved road* was [in use by 2016](https://web.archive.org/web/20210110184321/https://netflixtechblog.com/how-we-build-code-at-netflix-c5d9bd727f15); Spotify describes its [golden path](https://engineering.atspotify.com/2020/08/how-we-use-golden-paths-to-solve-fragmentation-in-our-software-ecosystem) (2020) as an opinionated, supported route. Teams can leave it, but take on responsibility for their alternatives and lose that support. For our comparison with Microsoft's app-hosting approach, see [open build, managed run](#open-build-managed-run).
...
Microsoft does not use one canonical word for this. GitHub calls the Copilot SDK a *"production-tested agent runtime,"* Copilot Studio's newer agent experience refers to an *"enhanced orchestration runtime,"* and Microsoft Agent Framework publishes the clearest first-party definition: *"An agent harness is the runtime scaffolding that turns a language model into an agent that can perform work"* ([Agent Harness](https://learn.microsoft.com/en-us/agent-framework/concepts/harness), retrieved 2026-08-17). Different words, same architectural slot. Knowing that saves you an argument in a design review. GitHub's own name for its production harness is the **[Copilot agent runtime](#copilot-agent-runtime)**, and GitHub says it now sits under Copilot Cowork, Copilot Studio, and the Office apps as well as GitHub Copilot. Copilot Studio, for its part, says its app creation *"is powered by the GitHub Copilot harness."*
...
**Microsoft Foundry**
The unified brand for Microsoft's AI development and management platforms, spanning Cloud (Microsoft Foundry (Azure)), Client (Windows AI Foundry), and Edge (Foundry Local). It provides a consistent toolchain for model selection, agent orchestration, and observability ([What is Microsoft Foundry?](https://learn.microsoft.com/en-us/azure/foundry/what-is-foundry), retrieved 2026-08-17). The new Foundry portal reached general availability in March 2026 for core scenarios (model discovery, agent development, evaluations, fine-tuning, red teaming, quota management). **Platform note:** Microsoft Foundry (Azure) is available in two portal experiences: **[Foundry (classic)](#f)** (hub-based, documented at `/azure/foundry-classic/`) and the new Foundry portal. They do **not** have feature parity. Documentation uses `?view=foundry-classic` or `?view=foundry-nextgen` parameters. **Service distinction:** Microsoft Foundry is the PLATFORM (portal, model catalog, prompt flow, evaluations). Microsoft documents **"more than 1,900 models"** in its catalog. [Foundry Agent Service](#f) is an OPTIONAL managed runtime within the platform for hosting agents. You can use Foundry without Agent Service by deploying custom code. See Decision Framework Q4 for platform vs service guidance.

**Microsoft IQ**
Microsoft's enterprise intelligence layer for agents and copilots: Foundry IQ for enterprise knowledge (mixed GA/Preview), Work IQ for work context (APIs GA; MCP Preview), Fabric IQ for business semantics and analytics (workload Preview; Chat, Cowork, and MCP integrations GA on Learn), and Web IQ for public web grounding (Limited Access). Check the component's status, not just the umbrella brand. The [Microsoft IQ hub](https://learn.microsoft.com/en-us/microsoft-iq/) lists *"four interconnected capabilities"*. [What is Foundry IQ?](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/what-is-foundry-iq), updated 2026-08-01, instead lists *"three IQ workloads"*, omitting Web IQ. The four-part list here follows the hub.

**Microsoft Scout (now Autopilot; Frontier / private preview)**
The former name of the Copilot app's [Autopilot](#autopilot), per Jared Spataro's 2026-09-25 Official Microsoft Blog post. [Learn's Scout documentation](https://learn.microsoft.com/en-us/microsoft-scout/) describes a desktop app with Frontier, Intune, and licensing requirements. Those requirements apply to Scout; their applicability to cloud-hosted Autopilot is not established as of 2026-09-25. See [Technologies]({{ '/docs/technologies#autopilot' | relative_url }}) before planning deployment.
...
Coordination among multiple agents. Keep **direct delegation** (A2A), **hierarchical handoff**, and **deterministic orchestration** separate: they solve different control problems and carry different lifecycle labels. Agent Framework provides GA code-first orchestration patterns; incoming Foundry A2A endpoints are Preview. A single agent triggered by an event is not multi-agent orchestration ([Agent Framework orchestration patterns](https://learn.microsoft.com/en-us/agent-framework/workflows/orchestrations/)).

## N

**NLWeb**
An open project where web endpoints act as MCP servers, allowing websites to provide conversational interfaces and be easily discoverable by AI agents ([NLWeb](https://github.com/nlweb-ai/NLWeb), retrieved 2026-08-17). NLWeb has since moved out of Microsoft's GitHub organization and is developed in the open at `nlweb-ai/NLWeb`.

## O

**Open build, managed run**
{: #open-build-managed-run }
Microsoft's phrase for separating app creation from app hosting. Builders can use supported authoring tools while Microsoft operates the host and IT governs the apps. David Blyth's announcement puts it plainly: *"Central governance should not require centralized creation"* ([Build where you want, run with confidence](https://www.microsoft.com/en-us/copilot/blog/copilot-studio/build-where-you-want-run-with-confidence-now-microsoft-hosts-and-manages-the-code-created-by-copilot/), published 2026-09-25).

The [golden path](#golden-path) comparison is our analogy, not a Microsoft taxonomy: centrally operated platform services reduce what builders have to run. The builder still owns the app's behavior, tests, and support. Managed hosting does not complete that review.

**Orchestration**
Coordinating agents, workflows, and function calls to execute multi-step solutions, often implemented with Agent Framework workflows that provide routing, state management, and human-in-the-loop checkpoints ([Microsoft Agent Framework](https://learn.microsoft.com/en-us/agent-framework/overview/), updated 2026-02-20).

## P

**Planetary Computer Pro**
A managed platform for geospatial insights that allows customers to ingest, manage, and analyze private geospatial data within Azure, integrating with Fabric and third-party tools ([Microsoft Planetary Computer Pro](https://learn.microsoft.com/en-us/azure/planetary-computer/), retrieved 2026-08-17).

**Plugin**
An extension package whose contents depend on the host. [Cowork plugins](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/cowork-admin-governance) come from the Microsoft 365 App Store and add skills and connectors. Name the host product: "plugin" alone does not identify an API or permission model.

The new Copilot plugin registry is described as *"Rolling out now and generally available across surfaces in the coming weeks"* in Jared Spataro's 2026-09-25 [Official Microsoft Blog post](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/). Its relationship to the Agent Store and MCP connectors is not stated in that announcement or the Learn pages checked as of 2026-09-25. See [Technologies]({{ '/docs/technologies#the-new-copilot-app-september-2026-read-the-status-column-first' | relative_url }}) for the rollout statements.

**Prompt Flow**
Microsoft Foundry (Azure)'s visual DAG environment for orchestrating LLMs, prompts, and Python tools, comparing prompt variants, collaborating across teams, and deploying flows as managed endpoints ([Prompt flow in Microsoft Foundry portal (classic)](https://learn.microsoft.com/en-us/azure/foundry-classic/concepts/prompt-flow), updated 2026-02-27). **Note:** Prompt Flow is available only in the Foundry (classic) portal. For new projects, use [Agent Framework Workflows](https://learn.microsoft.com/en-us/agent-framework/workflows/).
...
**Detection needs a sanctioned alternative.** [Copilot Managed Runtime (Preview)](#copilot-managed-runtime) gives teams a governed host with an app inventory and Entra sign-in. Confirm the effective connector and sharing policies before using it; a sanctioned host does not make an app's logic safe.

**Skills/Tools**
Function tools, hosted services, or built-in capabilities that agents attach at construction or per run, enabling actions like web search, file retrieval, or code execution within Agent Framework ChatClientAgent and ChatAgent implementations ([Agent Tools](https://learn.microsoft.com/en-us/agent-framework/agents/tools/), updated 2026-02-13).
...
**USL and UBB (user subscription license, usage-based billing)**
{: #usl-and-ubb }
Two parts of Copilot billing, not an either/or choice. The **USL** is the fixed-price foundation for everyday AI. **UBB** adds metered advanced AI and frontier models through Copilot Credits; those experiences *"require a USL for access"*. Learn's examples include Cowork, Code, Autopilot, *"new agentic experiences in SharePoint"*, and Astra and Fable. Its [Cowork model list](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/cowork-models) identifies the latter as OpenAI's GPT 6 Astra and Anthropic's Claude Fable 5.1. The USL usage-limit path is marked *"coming soon"*. Budget seats by population and credits by workload ([USL and UBB](https://learn.microsoft.com/en-us/microsoft-365/copilot/user-subscription-license-usage-based-billing), updated 2026-09-25).

## V

**Vibe coding**
{: #vibe-coding }
Coined by Andrej Karpathy on 2025-02-02 for working with AI while you *"forget that the code even exists"* ([original post](https://x.com/karpathy/status/1886192184808149383)). You accept generated code without reading it and steer by trying the result. Karpathy scoped it to *"throwaway weekend projects"*. That is different from [agentic engineering](#a), where tests, review, and other verification evidence support code people will depend on.

Usage has broadened. [Collins](https://blog.collinsdictionary.com/language-lovers/collins-word-of-the-year-2025-ai-meets-authenticity-as-society-shifts/) named it Word of the Year 2025 and describes natural-language prompting of AI to write code. Ask what verification someone means before treating the label as a method.

Microsoft uses both vocabularies. [Power Apps vibe (Preview)](https://learn.microsoft.com/en-us/power-apps/vibe/overview) adopts the term, and the [September 2026 Power Platform blog](https://www.microsoft.com/en-us/power-platform/blog/power-apps/whats-new-in-power-platform-september-2026-feature-update/) announces GA for the model-driven app-builder skill using the phrase *"vibe code entire model-driven apps"*. GitHub explicitly uses it in [Vibe coding with GitHub Copilot](https://github.blog/news-insights/product-news/github-copilot-agent-mode-activated/). The September 2026 Copilot app-building posts cited here use *natural language app building* and *agentic app creation*. Those are source-specific choices, not opposing vendor and practitioner camps.

Microsoft also used *vibe working* in Sumit Chauhan's [2025-09-29 announcement](https://www.microsoft.com/en-us/copilot/blog/2025/09/29/vibe-working-introducing-agent-mode-and-office-agent-in-microsoft-365-copilot/) for Agent Mode in Office apps and Office Agent in Copilot chat. The [release notes](https://learn.microsoft.com/en-us/microsoft-365/copilot/release-notes) also use *Edit with Copilot* for app editing; that is not evidence of a formal rename.
...
([Work IQ overview](https://learn.microsoft.com/en-us/microsoft-365/copilot/extensibility/work-iq/), updated 2026-06-17; [Announcing the new Work IQ APIs](https://www.microsoft.com/en-us/copilot/blog/2026/06/02/announcing-the-new-work-iq-apis/), published 2026-06-02; [Work IQ MCP overview (preview)](https://learn.microsoft.com/en-us/microsoft-agent-365/tooling-servers-overview), updated 2026-07-14).
