...
Connecting custom or fine-tuned language models to Microsoft AI platforms. The term means two different things depending on where you are standing: **(1) prompt-level BYOM** - point a prompt at an Azure AI Foundry model deployment, available within Copilot Studio, Power Apps, and Power Automate, and **(2) Microsoft Foundry BYOM** - deploy and manage fine-tuned models in the Foundry catalog. Microsoft folded the former AI Builder prompt surface into Copilot Studio, so older references to *AI Builder BYOM* now land here. Prompt-level BYOM needs a chat completions endpoint, and the GPT-5 family and later models are not currently supported ([Bring your own model for your prompts](https://learn.microsoft.com/en-us/microsoft-copilot-studio/bring-your-own-model-prompts), updated 2026-07-14). *When to use:* BYOM is appropriate when base models lack domain expertise, require specific tone/format, or must comply with specialized regulatory requirements. See [Feature Comparison]({{ '/docs/feature-comparison' | relative_url }}) for platform-specific capabilities.
