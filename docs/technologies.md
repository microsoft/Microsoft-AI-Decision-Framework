...
- **New agent experience (Production-ready preview):** A rebuilt authoring and runtime path running alongside the classic experience. Learn describes it as *"a redesigned authoring and runtime environment"* on an **enhanced orchestration runtime**, and states that the new experience uses that runtime *"for all agents, with no option to switch."* **Vocabulary warning:** Microsoft's CAT team blog calls this same technology the **"GitHub Copilot harness"**, a term that appears nowhere in Learn. Both names refer to the same thing; Learn is the citable authority. (What's new in Copilot Studio - Updated: 2026-07-15; [Classic vs new](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/classic-vs-new) - Updated: 2026-07-10)
- **The two experiences do not convert, in either direction.** Verbatim: *"You can't transfer agents created in the new experience to the classic experience. You also can't transfer agents created in the classic experience to the new experience. This limitation exists because the two experiences use fundamentally different architectures and orchestration runtimes."* **No deprecation of the classic experience is announced**, and Learn describes it as fully supported. Treat this as a fork to decide deliberately, not a migration to schedule. ([Classic vs new](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/classic-vs-new) - Updated: 2026-07-10)
- **What the classic experience still does better.** Learn is direct about the trade: the classic experience offers *"explicit conversation flows with conditions, branching, and node-based logic,"* while the new experience *"uses agent instructions and reasoning instead of explicit flows."* If you need *"precise control over every step of a conversation, the classic experience provides more deterministic options."* Learn also states plainly that the classic experience *"has a broader set of configurable features"* without enumerating them. ([Classic vs new](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/classic-vs-new) - Updated: 2026-07-10)
- **Skills (new experience, preview):** Modular, on-demand instruction sets. Learn documents a skill package as a ZIP containing *"A SKILL.md file containing YAML front matter (name, description) and Markdown instructions"* plus optional supporting files such as scripts. Markdown and files rather than a proprietary artifact, which makes a skill unusually portable between tools. **Disambiguation:** these are not the older Bot Framework "skills," which were callable services. Same word, unrelated construct. ([Skills overview](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/skills-overview) - Updated: 2026-06-30)
- **Microsoft IQ and Foundry IQ connectivity:** Ground agents with Microsoft 365 organizational context (emails, calendar, files, chats, people) and optionally connect to a Microsoft Foundry knowledge base. The permission model carries over into the new experience: *"All access respects the user's existing permissions. Your agent can only access data the user is authorized to see."* ([Use Microsoft IQ](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/use-microsoft-iq) - Updated: 2026-06-30, labeled a preview feature). The what's-new page separately labels **"Foundry IQ connect (Preview)"**. Do not inherit Copilot Studio's GA surfaces for this connection. (What's new in Copilot Studio - Updated: 2026-07-15)
- **Connected agents in the new experience are Copilot Studio only.** Learn states you *"can currently only connect other agents built in Copilot Studio"* there. A2A reached GA in April 2026 for the classic experience, so this is a capability that exists in the product but not yet on this path. Check before designing a multi-agent topology that assumes external delegation. ([Connected agents](https://learn.microsoft.com/en-us/microsoft-copilot-studio/agents-experience/connected-agents) - Updated: 2026-06-30)
- **Agent sandbox (new experience):** An execution environment where the agent writes and runs code to do the work models are bad at, such as exact arithmetic and building files byte for byte. Two properties matter for governance. **There is no outbound network path:** per the CAT team, *"Code running there cannot call an API, send an email, or write a file to SharePoint, no matter what it imports."* External access happens only through configured tools and knowledge, so *"everything the agent does externally stays within your governance controls and data policies."* And **it is temporary**: a file the agent produces must be delivered somewhere durable in the same turn, because it will not be there later, and agent memory persists facts rather than files. (CAT team blog, *The New Copilot Studio Agent Sandbox*, 2026-07-20)
...
- **Generative orchestration (GA):** The reasoning engine that decides which topics, tools, and knowledge to invoke **within the classic experience**. It is **GA and the default for new classic-experience agents**, with the older classic orchestration remaining for deterministic topic routing. Note the three-way distinction: classic orchestration and generative orchestration are modes *inside* the classic experience; the enhanced orchestration runtime belongs to the new experience and is not one of them. (What's new in Copilot Studio - Updated: 2026-07-15)
...
{: .warning }
> **Do not read "Copilot Studio is GA" as "everything in Copilot Studio is GA."** The status that matters is always per feature, and the new agent experience is the sharpest current example: Learn labels it **production-ready preview**, which permits production use but is not GA, while several capabilities inside it sit a tier lower at plain **preview**, which Microsoft says is *"not meant for production use."* A roadmap entry announcing general availability is not the same as Learn stating it; when the two disagree, cite Learn. **Verified GA:** generative orchestration (default for new classic-experience agents), computer use (May 2026), A2A (April 2026), agent evaluations, scheduled prompts (2026-07-01). **Verified preview:** Copilot Tuning (early access preview), plus the what's-new page's own **"Connect other agents (Preview)"** and **"Foundry IQ connect (Preview)"**. For **autonomous/triggered agents, child (inline) agents, connected Copilot Studio agents, Foundry agents, Fabric data agents, and M365 Agents SDK agents inside Copilot Studio, Microsoft does not state a status**, and where Microsoft is silent, so are we. Check the current page before you commit; do not infer GA from the absence of a preview banner.
