...
| **4. Complexity Assessment** | Technical complexity evaluation | [Evaluation Criteria: Complexity]({{ '/docs/evaluation-criteria#1-complexity-assessment-architectural-load' | relative_url }}) |
| **5. Budget & Timeline** | Cost and time-to-production paths | [Evaluation Criteria: Budget & Time]({{ '/docs/evaluation-criteria#3-budget-assessment' | relative_url }}) |
| **6. Governance & Compliance** | Security and compliance requirements | [Evaluation Criteria: Governance]({{ '/docs/evaluation-criteria#5-governance--compliance-the-security-perimeter' | relative_url }}) |
...
#### Business Builder: The Natural-Language Build Path (Bucket 3)
{: .no_toc }

**The Trade-off: Speed of the build vs. ownership of the app.** The first three rows are alternative build paths, not steps. Cowork and Copilot Studio apps run on Copilot Managed Runtime, and Microsoft says Code's shared apps use the same foundation; the *Run it* and *Hand it over* rows describe that host. The last row is Power Platform's own path, which hands over by fork instead. The Copilot Studio and Power Apps vibe pages both warn: *"Preview features aren't meant for production use and might have restricted functionality."* How to choose among them is worked through in [Scenario 11]({{ '/docs/scenarios#scenario-11-business-teams-building-their-own-apps' | relative_url }}).

| Step | Technology | Status and limits | Documentation |
|------|------------|-------------------|---------------|
| **Describe it: just you, or a quick team tool** | **Copilot Code** (Code in the Copilot app) | **Frontier rollout; Microsoft states the timing two ways.** Official Microsoft Blog, 2026-09-25: *"Code is rolling out to Frontier at the end of the month, with broad availability in the coming weeks"*; earlier in the same post, *"Home and Code will start rolling out in our Frontier program in the coming weeks"*. The same day's runtime announcement: *"Microsoft entry points are already live with Copilot Cowork, Copilot Code, and Copilot Studio"*. Feature and rollout details are blog-sourced; no dedicated Learn feature overview was found as of 2026-09-25, though Learn documents Code's usage-based billing on Copilot Credits. Learn's list of runtime creation paths does not include Code. | [Official Microsoft Blog](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/) \| [Runtime announcement](https://www.microsoft.com/en-us/copilot/blog/copilot-studio/build-where-you-want-run-with-confidence-now-microsoft-hosts-and-manages-the-code-created-by-copilot/) \| [USL and UBB](https://learn.microsoft.com/en-us/microsoft-365/copilot/user-subscription-license-usage-based-billing) |
| **Describe it: a team app over Microsoft 365 content** | **App skill** in Copilot Cowork | **Frontier program only.** *"Cowork requires a Microsoft Copilot license, but the subscription doesn't include Cowork usage."* Shared by link. Cowork's page: *"Anyone with the link can open and use the app, including all its data"*. The runtime FAQ: sharing *"doesn't grant access to the app's underlying data"*. The pages conflict; test with an account that lacks source access before sharing. | [Use Copilot Cowork](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/use-cowork) \| [Admin overview](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/) |
| **Build it: one piece of a business process** | **Copilot Studio** apps | **Preview.** *"Microsoft is gradually rolling out apps in Copilot Studio (Preview). This feature might not be available in your environment yet."* App creation is *"powered by the GitHub Copilot harness"*, and using, building, previewing, and testing apps can consume Copilot Credits. *"The code view is read-only."* Sharing an app *"might not automatically share its underlying data"*. The apps documentation reviewed for this update gives no example of an app invoking an agent or agent flow. | [Apps overview](https://learn.microsoft.com/en-us/microsoft-copilot-studio/apps-experience/apps-overview) \| [FAQ](https://learn.microsoft.com/en-us/microsoft-copilot-studio/faq-apps) |
| **Run it** | **Copilot Managed Runtime** | **Preview.** A governed host, not a completed governance review. Microsoft Entra sign-in. Connector governance depends on the environment group: with an advanced connector policy and *Advanced connector policies only* off, *"The most restrictive result applies"*; with that setting on and no ACP, *"Data policies are ignored. Without an ACP, connectors aren't restricted."* Sharing limits per environment group. One Apps inventory in the Microsoft 365 admin center, whose Data & tools view *"doesn't provide a complete view of exact destinations, dynamically resolved endpoints, actions that the app executed, or each user's permissions to the underlying data"*. Apps must be TypeScript or JavaScript single-page apps. Running an app takes Copilot Credits or a Power Apps Premium license; with Premium, credits still apply to separately billed services such as Work IQ APIs, or past the plan's API request limits. **Git pushes and code changes are not audited** in Purview. Automatic for eligible commercial-cloud tenants; sovereign clouds and GCC not stated as of 2026-09-25. | [Overview](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/) \| [Governance](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/governance) \| [Security and compliance](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/security-compliance) \| [FAQ](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/faq-about-apps) |
| **Hand it over** | **GitHub Copilot** with the Managed Runtime plugin, or the `ms` CLI | **Preview** (SDK, CLI, and plugin). Same Git repository, same runtime. The plugin *"also works with Claude Code and other coding agents through Open Plugins"*. Repository type is fixed when the app is created: platform-managed Git, or your own repository on GitHub.com or GitHub Enterprise Cloud (GitHub Enterprise Server and Azure DevOps aren't supported). With your own repository, *"your existing branch policies and pull request reviews continue to apply"*. | [SDK overview](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/) \| [GitHub Copilot quickstart](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/quickstart-github-copilot) |
| **Or describe it on the Power Platform side** | **Power Apps vibe** | **Preview.** Builds *"requirements, data, and full apps with generated code"*. Not available in a default environment; US, Australia, Asia, and India regions only, English only. Its own page says of Cowork and Copilot Studio app building: *"We recommend using these experiences moving forward."* Handoff is a fork: *"Converting a vibe app creates a new code app. The code app is disconnected from the original plan."* | [Overview](https://learn.microsoft.com/en-us/power-apps/vibe/overview) \| [Convert to a code app](https://learn.microsoft.com/en-us/power-apps/vibe/convert-vibe-to-code) |

{: .warning }
> **The handoff has a blind spot.** The *Hand it over* row continues the same repository; the Power Apps vibe row creates a separate code app. On the runtime, Purview lists Git pushes and code changes as *"Not audited"*. Bind the app to your own GitHub repository at creation and your branch policies and pull-request reviews keep applying. For the platform-managed default, the documentation we reviewed doesn't establish a review gate, so define one on purpose before the handoff.
...
- [Logic Apps agentic workflows concepts](https://learn.microsoft.com/en-us/azure/logic-apps/agent-workflows-concepts) (verified 2026-07-29)
- [What is Copilot Managed Runtime (preview)](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/) (prerelease documentation; three documented entry points: Cowork, Copilot Studio, SDK and CLI; verified 2026-09-25)
- [Copilot Managed Runtime default governance settings](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/governance) and [security and compliance](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/security-compliance) (lists "Git push / code change" as "Not audited"; verified 2026-09-25)
- [FAQ about Copilot Managed Runtime](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/faq-about-apps) (sharing does not grant data access, which conflicts with the Cowork page; limits of the Data & tools view; runtime eligibility is Copilot Credits or Power Apps Premium; verified 2026-09-25)
- [Copilot Managed Runtime overview and key concepts for admins](https://learn.microsoft.com/en-us/microsoft-365/admin/manage/apps/) (Cowork build licensing, Power Apps Premium exclusions, creation-path defaults; verified 2026-09-25)
- [Copilot Managed Runtime SDK overview](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/) and [GitHub Copilot quickstart](https://learn.microsoft.com/en-us/microsoft-365/managed-apps/developer/quickstart-github-copilot) (preview; repository options and the Claude Code note; verified 2026-09-25)
- [Use Copilot Cowork: build apps with the App skill](https://learn.microsoft.com/en-us/microsoft-365/copilot/cowork/use-cowork) (Frontier; page updated 2026-09-14, verified 2026-09-25)
- [Copilot Studio apps overview](https://learn.microsoft.com/en-us/microsoft-copilot-studio/apps-experience/apps-overview) (preview, rolling out gradually; verified 2026-09-25) and [FAQ for apps in Microsoft Copilot Studio](https://learn.microsoft.com/en-us/microsoft-copilot-studio/faq-apps) (read-only code view; sharing might not share underlying data; verified 2026-09-25)
- [Overview of the new Power Apps vibe experience](https://learn.microsoft.com/en-us/power-apps/vibe/overview) (preview; prerelease documentation updated 2026-09-08, with a banner recommending Cowork and Copilot Studio app building going forward; verified 2026-09-25) and [Convert a vibe app to a code app](https://learn.microsoft.com/en-us/power-apps/vibe/convert-vibe-to-code) (conversion creates a separate, disconnected code app; verified 2026-09-25)
- [Introducing the new Copilot with Home, Code and Autopilot](https://blogs.microsoft.com/blog/2026/09/25/introducing-the-new-copilot-with-home-code-and-autopilot/) (Official Microsoft Blog, Jared Spataro, 2026-09-25; Code's feature description and rollout timing)
- [Microsoft Copilot Managed Runtime: Enterprise code execution for Microsoft 365](https://www.microsoft.com/en-us/copilot/blog/copilot-studio/build-where-you-want-run-with-confidence-now-microsoft-hosts-and-manages-the-code-created-by-copilot/) (Microsoft Copilot blog, 2026-09-25; entry points "already live" with Cowork, Code, and Copilot Studio)
- [Understanding the user subscription license (USL) and usage-based billing (UBB)](https://learn.microsoft.com/en-us/microsoft-365/copilot/user-subscription-license-usage-based-billing) (Code's usage-based billing; verified 2026-09-25)
...
| **3-6 Months** | Microsoft Foundry (Azure) custom solutions, complex orchestration | [(evaluation-criteria)]({{ '/docs/evaluation-criteria#4-time-to-production-the-runway' | relative_url }}) |
